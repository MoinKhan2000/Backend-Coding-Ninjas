...
class ExpenseRepository {
  ...
  async updateTagInExpense(id, oldTag, newTag) {
    ...
    const result = await db.collection(this.collectionName).updateOne(
      { _id: new ObjectId(id), tags: oldTag }, // find document by ID and old tag
      { $set: { "tags.$": newTag } } // set the tag to the new value
    );
    return result;
  }
  ...
  async deleteTagFromExpense(id, tag) {
    ...
    const result = await db.collection(this.collectionName).updateOne(
      { _id: new ObjectId(id) },
      { $pull: { tags: tag } } // pull (remove) the tag from the array
    );
    return result;
  }
}
