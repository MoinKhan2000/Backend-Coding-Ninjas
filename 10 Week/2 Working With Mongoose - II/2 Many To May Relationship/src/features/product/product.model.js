...
export default class ProductModel {
    ...
    static addProduct(id, name, desc, imageUrl, category, price, sizes) {
        ...
        const newProduct = new ProductModel(id, name, desc, imageUrl, categories, price, sizes);
    ...
    static rateProduct(userId, id, rating) {
        // Validate User and product
        let user = UserModel.getAllUsers().find(user => user.id === userId)
        if (user) {
            const index = products.findIndex(product => product.id === id);
            if (index !== -1) {
                // Check if the user has already rated the product
                const isAlreadyRated = products[index].rating.findIndex(r => r.userId == userId)
                if (isAlreadyRated >= 0) {
                    products[index].rating[isAlreadyRated] = { userId, rating }
                    // return products[index];
                    return null
                }
                products[index].rating.push({ userId, rating });
                // return products[index];
                return null
            }
            else {
